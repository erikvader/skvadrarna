...
void node_cut(tree_node **a, tree_node *a_par, tree_node **b, tree_node *b_par){
   tree_node *temp = *a;
   *a = *b;
   *b = temp;
   if(*a != NULL){
...
void tree_rotate(tree_node *n, enum tree_rotate_dir dir){
   ...
   if(dir == ROTATE_LEFT){
      node_cut(&a->right, a, &b->left, b);
   }else{
      node_cut(&a->left, a, &b->right, b);
   }
   update_height(b);
   update_height(a);
}
